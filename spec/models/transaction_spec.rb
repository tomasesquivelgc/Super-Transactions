...
RSpec.describe Transaction, type: :model do
  it 'is valid with valid attributes' do
    ...
    transaction = Transaction.new(name: 'Example Transaction', amount: 100.00, user:)
    expect(transaction).to be_valid
  end

  it 'is not valid without a name' do
    user = User.create(name: 'example_user', email: 'user@example.com', password: 'password')
    transaction = Transaction.new(name: nil, amount: 100.00, user:)
    expect(transaction).not_to be_valid
  end

  it 'is not valid without an amount' do
    user = User.create(name: 'example_user', email: 'user@example.com', password: 'password')
    transaction = Transaction.new(name: 'Example Transaction', amount: nil, user:)
    expect(transaction).not_to be_valid
  end
  ...
  it 'has a factory bot' do
    transaction_bot = create(:transaction)
    expect(transaction_bot).to be_valid
  end
end
